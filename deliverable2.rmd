...
```{r}
CA[, "Diploma_per_cap"] <-  scales::percent((as.numeric(unlist(CA[,"Diploma"])) / as.numeric(unlist(CA[,"Population"]))))
CA[, "Violence_per_cap"] <-  scales::percent((as.numeric(unlist(CA[,"Violent_crime_total"])) / as.numeric(unlist(CA[,"Population"]))))
...
train = violence[sampling, ]
test = violence[-sampling, ]
predict_violence_model <- lm(data=train, formula = Violence_per_cap ~ Diploma)
